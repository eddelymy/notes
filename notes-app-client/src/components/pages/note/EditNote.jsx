...
import ReactColor from '../../common/ReactColor'
import ReactEmoji from '../../common/ReactEmoji'
import { UrlPage } from "../../common/UrlPage"
import noteService from '../../../service/note/note.service'
import { setErrors } from '../../../helpers/error'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faBold, faFileImport, faItalic, faLink, faListOl, faListUl, faUnderline } from '@fortawesome/free-solid-svg-icons'
...
export default function EditNote() {
  ...
  async function findNoteById(){
    try{
      const {data} = await noteService.finNoteById(id)
      setTitle(data.title)
  ...
  return (
    <div className='flex flex-col'>
      <UrlPage pages={['Modifier la note ']} />
      <div className="mt-10 width-full border border-1 bg-white rounded-md flex items-center max-w-full p-5 w-full">
        <form className="w-full">
          ...
          <div className='flex mt-4 justify-between'>
            <button type='button' className='mr-3 text-blue-500' onClick={()=>navigate('/notes')}>Page précédente</button>
            <div>
